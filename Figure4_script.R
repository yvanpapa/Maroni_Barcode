...
session_Fig4<-"E:/BARCODE_MARONI/SCRIPTS_AND_DATA/FIGURE4"
setwd(session_Fig4)

#IDEA FOR AN EVEN LIGHTER VERSION: USE SA_RIVS 15S INSTEAD OF 30S?
...
####COLORED VERSION PRODUCES A VERY HEAVY PDF. NOT OPENABLE WITH ADOBE OR INKSCAPE####
...
#layer(sp.polygons(maroni_basins))+layer(sp.polygons(maroni_rivs2, col = "dodgerblue3", lwd=1))->lay1   #basins limits and rivers
#extend(rnorm_xyz_total.masked,ext_basemap)->rnorm_xyz_total.masked2
#extend(count_total.masked,ext_basemap)->count_total.masked2
...
#r2l2+as.layer(rast1+lay1,under=T)->plot
#print(plot,split=c(2,1,2,1),newpage=F)
#dev.print(pdf, paste0('Figure4.pdf'))
#dev.print(png, paste0('Figure4.png'),width = 1024, height = 768)
#dev.off()
